...
int
is_tx_space_available(xaxiemacif_s *emac)
{
...
int
process_sent_bds(XAxiDma_BdRing *txring)
{
...
axidma_send_handler(void *arg)
{
	...
	XIntc_AckIntr(xtopologyp->intc_baseaddr, 1 << xaxiemac->Config.AxiDmaTxIntr);
...
static void
_bd_csum_disable(XAxiDma_Bd *bd)
{
	XAxiDma_BdWrite((bd), XAXIDMA_BD_USR0_OFFSET,
		(XAxiDma_BdRead((bd), XAXIDMA_BD_USR0_OFFSET)
		 & ~1));
...
static void
_bd_csum_set(XAxiDma_Bd *bd, u16_t tx_csbegin, u16_t tx_csinsert, u16_t tx_csinit)
{
	u32_t app1;
...
static void
_bd_fullcsum_disable(XAxiDma_Bd *bd)
{
	XAxiDma_BdWrite((bd), XAXIDMA_BD_USR0_OFFSET,
		(XAxiDma_BdRead((bd), XAXIDMA_BD_USR0_OFFSET)
		 & ~3));
...
static void
_bd_fullcsum_enable(XAxiDma_Bd *bd)
{
	XAxiDma_BdWrite((bd), XAXIDMA_BD_USR0_OFFSET,
		(XAxiDma_BdRead((bd), XAXIDMA_BD_USR0_OFFSET)
		 | 2));
...
XStatus
axidma_sgsend(xaxiemacif_s *xaxiemacif, struct pbuf *p)
{
	struct pbuf *q;
	int n_pbufs;
	...
	if (XAxiEthernet_IsTxPartialCsum(&xaxiemacif->axi_ethernet)) {
		if (p->len > sizeof(struct ethip_hdr)) {
			...
			if (htons(ehdr->eth.type) == ETHTYPE_IP && proto == IP_PROTO_TCP) {
				...
				_bd_csum_set(txbdset, tcp_payload_offset, csum_insert_offset, htons(~csum_init));
			}
		}
	}
#endif
...
#if XPAR_AXIETHERNET_0_TXCSUM == 2
	_bd_fullcsum_disable(txbdset);

	if (p->len > sizeof(struct ethip_hdr)) {
		_bd_fullcsum_enable(txbdset);
	}
#endif
	/* enq to h/w */
...
void
_setup_rx_bds(XAxiDma_BdRing *rxring)
{
	XAxiDma_Bd *rxbd;
	int n_bds, i;
	XStatus Status;
...
int
is_tcp_ip_checksum_valid(XAxiDma_Bd *rxbd) {
 ...
 */
int
is_checksum_valid(XAxiDma_Bd *rxbd, struct pbuf *p) {
	struct ethip_hdr *ehdr = p->payload;
	u8_t proto = IPH_PROTO(&ehdr->ip);
...
axidma_recv_handler(void *arg)
{
	...
	XIntc_AckIntr(xtopologyp->intc_baseaddr, 1 << xaxiemac->Config.AxiDmaRxIntr);
...
#ifdef USE_JUMBO_FRAMES
			XCACHE_INVALIDATE_DCACHE_RANGE(p->payload, XAE_MAX_JUMBO_FRAME_SIZE);
#else
	...
	if (is_tcp_ip_checksum_valid(rxbd)) {
		...
		/* _setup_rx_bds -> use XAxiDma_BdRingGetFreeCnt */
...
XStatus
init_axi_dma(struct xemac_s *xemac)
{
	...
	for (i = 0; i < XLWIP_CONFIG_N_RX_DESC; i++) {
		...
		/*
		 * Setup the BD. The BD template used in the call to XAxiEthernet_SgSetSpace()
		 * set the "last" field of all RxBDs. Therefore we are not required to
		 * issue a XAxiDma_Bd_SetLast(rxbd) here.
		 */
		XAxiDma_BdSetBufAddr(rxbd, (u32)p->payload);
#ifndef XPAR_AXIDMA_0_ENABLE_MULTI_CHANNEL
		XAxiDma_BdSetLength(rxbd, p->len);
	...
	Status = XAxiDma_BdRingSetCoalesce(TxRingPtr, XLWIP_CONFIG_N_TX_COALESCE, 0x1);
	if (Status != XST_SUCCESS) {
		LWIP_DEBUGF(NETIF_DEBUG, ("Error setting coalescing settings\r\n"));
		return XST_FAILURE;
	}

	Status = XAxiDma_BdRingSetCoalesce(RxRingPtr, XLWIP_CONFIG_N_RX_COALESCE, 0x1);
	if (Status != XST_SUCCESS) {
		LWIP_DEBUGF(NETIF_DEBUG, ("Error setting coalescing settings\r\n"));
		return XST_FAILURE;
	}
